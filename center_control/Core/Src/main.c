...
int16_t count_tim4 = 0; // 记录 TIM4 的计数值
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (flag_temp_update) // 如果温度数据已更新
    {
      // 计算 PID 输出
      int control_output = (int)PID_Compute(&temp_pid, Sys_Temperatures[1]);
      // 执行输出
      __HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_2, 1700 - control_output);
      flag_temp_update = 0; // 重置温度更新标志，等待下一次温度数据更新
    }
    if (count_tim4 % 1000 == 0)
    {
