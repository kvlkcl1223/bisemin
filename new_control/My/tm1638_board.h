...
typedef struct
{
    ...
    uint8_t key_states[KEY_MAX_NUM];                 // 记录按键稳定状态(消抖后)
    TM1638_KeyCallback_t key_callbacks[KEY_MAX_NUM]; // 按键按下时的回调函数
    uint8_t debounce_cnt[KEY_MAX_NUM];               // 消抖计数器: 连续相同读数次数
} TM1638_HandleTypeDef;

/* 全局 TM1638 设备句柄, 供所有任务直接引用 */
extern TM1638_HandleTypeDef htm1638;
...
/* 一键硬件初始化: 配置底层引脚 + 初始化 TM1638 + 设置默认亮度 */
