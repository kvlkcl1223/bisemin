 ...
 ******************************************************************************
 * @file    calib_mode.h
 * @brief   自动化标定模�?�?固定占空比扫描，记录稳态平均温�?
 * @author  Bisemin Team
 * @version V1.0.0
 * @date    2026-07-02
 *
 * @details
 * �?+0.30（制冷）�?-0.40（加热）�?-0.02 步进，共 36 步�? * 每一步设定固定占空比后等待温度稳定（波动 < CALIB_STABLE_THRESHOLD
...
/** @brief 标定过程温度采样间隔 (ms)，应�?ControlTask 周期一�?*/
...
#define CALIB_WINDOW_SAMPLES (CALIB_STABLE_SAMPLES)
  ...
  /*
   * 快速测试模�?�?取消下行注释即可启用
   * 仅标�?1 步（duty=0.0），其余 45 步复制填充，~10 秒完�?
   * 用于验证 Flash 写入是否正常
   */
...
#define CALIB_SCAN_COUNT CALIB_DUTY_COUNT
  ...
  {
    CALIB_IDLE = 0,    /**< 空闲，等待启�?*/
    CALIB_INIT,        /**< 初始化：准备 DRV 通道，擦�?Flash */
  ...
  {
    float duty;      /**< 占空�?*/
    float temp_ch0;  /**< 通道0 稳态平均温�?(°C) */
    float temp_ch1;  /**< 通道1 稳态平均温�?(°C) */
    uint8_t valid;   /**< 是否有效 (1 = 稳定达到, 0 = 超时/故障) */
    uint8_t settled; /**< 是否达到稳�?(1 = �? 0 = 超时未稳�? */
    uint16_t reserved;
  } CalibStep_t;

  /* Flash 存储数据结构 ---------------------------------------------------------*/

#define CALIB_FLASH_MAGIC 0x42495346UL /**< "BISF": asymmetric duty calibration table */

/** @brief Flash 存储区偏移：Cell 0（硬�?4KB �?126�?*/
#define CALIB_FLASH_OFFSET_CELL0 0U

/** @brief Flash 存储区偏移：Cell 1（硬�?4KB �?127，与 Cell 0 不在同一页） */
#define CALIB_FLASH_OFFSET_CELL1 4096U
  ...
  /** @brief 标定模式下数码管显示标志�? = 显示占空比，0 = 显示温度 */
  extern volatile uint8_t g_calib_show_duty;

  /** @brief 标定是否处于运行中（�?LED 灯判断，1 = 运行中） */
