 ...
 * 内层/从路 duty �?inner_ratio 跟随外层�?
 */
/** @brief Cell 0 外层测温输入索引，默认实�?CH1�*/
#define APP_CONTROL_CELL0_TEMP_OUTER 0U
/** @brief Cell 0 内层测温输入索引，默认实�?CH2�*/
#define APP_CONTROL_CELL0_TEMP_INNER 1U
/** @brief Cell 0 外层/主路 DRV 索引，默认实�?DRV1�*/
#define APP_CONTROL_CELL0_DRV_OUTER 0U
/** @brief Cell 0 内层/从路 DRV 索引，默认实�?DRV2�*/
#define APP_CONTROL_CELL0_DRV_INNER 1U
/** @brief Cell 0 制冷时内�?duty 跟随比例，inner = outer * ratio�*/
#define APP_CONTROL_CELL0_COOLING_INNER_DUTY_RATIO 0.70f
/** @brief Cell 0 加热时外�?duty 跟随比例，outer = inner * ratio�*/
...
/** @brief Cell 1 外层测温输入索引，默认实�?CH3�*/
#define APP_CONTROL_CELL1_TEMP_OUTER 2U
/** @brief Cell 1 内层测温输入索引，默认实�?CH4�*/
#define APP_CONTROL_CELL1_TEMP_INNER 3U
/** @brief Cell 1 外层/主路 DRV 索引，默认实�?DRV3�*/
#define APP_CONTROL_CELL1_DRV_OUTER 2U
/** @brief Cell 1 内层/从路 DRV 索引，默认实�?DRV4�*/
#define APP_CONTROL_CELL1_DRV_INNER 3U
/** @brief Cell 1 制冷时内�?duty 跟随比例，inner = outer * ratio�*/
#define APP_CONTROL_CELL1_COOLING_INNER_DUTY_RATIO 0.70f
/** @brief Cell 1 加热时外�?duty 跟随比例，outer = inner * ratio�*/
...
#define APP_CONTROL_CELL_SHARED_DUTY 0.0f
    ...
    typedef enum
    {
        ...
        APP_CONTROL_ERROR_PARAM  /**< 参数非法，例�?cell 编号越界�*/
    } AppControl_Status_t;
    ...
    /** @brief 投递给 ControlTask �?cell 控制命令�*/
    typedef struct
    {
        AppControlCommandType_t type;   /**< 命令类型：启动或停止�*/
        uint8_t cell;                   /**< 目标 cell 编号，范�?0..APP_CONTROL_CELL_COUNT-1�*/
        AppControlStopMode_t stop_mode; /**< 停止命令使用的停止方式，启动命令忽略�*/
    } AppControlCommand_t;

    /** @brief DRV8703 仿真开关，�?0 时跳过真�?DRV 初始�?读写，便于脱板调试�*/
    extern volatile uint8_t g_app_control_simulate_drv8703;
    /** @brief 电压检测仿真开关，�?0 时按电压正常处理�*/
    ...
    extern volatile uint8_t g_app_control_drv_config_verify_ok[APP_CONTROL_DRV_COUNT];
    extern volatile uint8_t g_app_control_drv_config_verify_attempts[APP_CONTROL_DRV_COUNT];
    extern volatile uint8_t g_app_control_drv_config_verify_mismatch_mask[APP_CONTROL_DRV_COUNT];
    extern volatile uint8_t g_app_control_drv_config_verify_reg_dump[APP_CONTROL_DRV_COUNT][DRV8703_REGISTER_COUNT];
    extern volatile DRV8703_Status_t g_app_control_drv_config_verify_reg_status[APP_CONTROL_DRV_COUNT][DRV8703_REGISTER_COUNT];
    /** @brief 最近一次发生故障的 DRV 索引�?xFF 表示暂无�*/
    extern volatile uint8_t g_app_control_last_drv_fault;
    /** @brief 最近一�?DRV 操作返回状态�*/
    ...
    /** @brief FAULT 引脚触发时是否出现全 �?0xFF 异常读数�*/
    ...
    /** @brief 每个 cell 最近判定异常的测温输入索引�?xFF 表示暂无�*/
    ...
    /** @brief 每路闭环 DRV 通道�?PID 计算触发频率，单�?Hz�*/
 ...
 * DRV8703 原始寄存器轮询�?
 *
 * 该轮询独立于故障快照，会定期尝试读取全部 5 �?DRV8703�?
 * 用于观察芯片通信状态和寄存器原始值�?
 */
...
#define APP_CONTROL_DRV_RAW_POLL_MS 200U
     ...
     * 注意：DRV 编号�?0 基索引，0=DRV1�?=DRV2�?=DRV3�?=DRV4�?=DRV5�?
     ...
     * 调试观察�?
     *
     *     g_app_control_drv_test_active_mask  // 当前仍在输出�?DRV 掩码
     *     g_app_control_drv_test_fault_mask   // 已检测到故障并关闭的 DRV 掩码
     *     g_app_control_drv_test_status[]     // 每路最近一次测试状�?
     */
    ...
    /** @brief 请求启动指定 cell �?PID 温控，由 ControlTask 异步执行�*/
    void Control_StartPid(uint8_t cell);
    /** @brief 请求正常停止指定 cell �?PID 温控，由 ControlTask 异步执行�*/
    void Control_StopPid(uint8_t cell);
    /** @brief 请求紧急停止指�?cell �?PID 温控�*/
