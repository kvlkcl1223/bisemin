...
static uint16_t s_frm_len         = 0U;   /* LEN + TYPE + SEQ + PAYLOAD (�?LEN) */
...
/**
 * @brief  �?payload 中取 key=value，拷贝到 dst 并补 \0
 * @return 1=找到, 0=未找�? */
    ...
    case PC_STATE_READ_LEN:
        ...
        if (s_len_idx >= 2U)
        {
            ...
            else
            {
                 ...
                 * �?LEN 写入 body_buf 开头，CRC 校验�?s_body_buf[0..] 包含 LEN
                 */
                s_body_buf[0] = s_len_buf[0];
                s_body_buf[1] = s_len_buf[1];
                s_body_idx    = 2U;               /* TYPE+SEQ+PAYLOAD 从偏�?2 开�?*/
...
void PcProto_SendState(uint8_t cell)
{
    ...
    t0_idx = AppControl_GetCellOuterTempIndex(cell);
    t1_idx = AppControl_GetCellInnerTempIndex(cell);
    t0 = (t0_idx < APP_CONTROL_CLOSED_LOOP_COUNT) ? g_app_control_pid_temp[t0_idx] : 0.0f;
    t1 = (t1_idx < APP_CONTROL_CLOSED_LOOP_COUNT) ? g_app_control_pid_temp[t1_idx] : 0.0f;
    duty_outer = AppControl_GetCellOuterDuty(cell);
    duty_inner = AppControl_GetCellInnerDuty(cell);
    ratio = AppControl_GetCellInnerDutyRatio(cell);
...
/**
 * @brief  发�?Cell 过程数据帧（Fix #5: 真正�?DATA 类型�? */
