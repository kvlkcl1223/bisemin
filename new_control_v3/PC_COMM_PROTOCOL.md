...
本文档定义上位机 Qt 软件�?MCU 之间的串口通信协议�?

设计目标�?

- MCU 是唯一控制真相源，负责 PID、程序升温、故障保护、DRV 输出和温度采集�?
- 面板�?Qt 软件地位平等，都是人机入口�?
- Qt 不直接控�?PWM，不计算 PID，只发送用户意图�?
- MCU 接收命令后修改内部状态，并通过状态帧广播�?Qt�?
...
这样普通模式和程序升温模式都可以由面板�?Qt 启动，控制逻辑仍然只有一套�?
...
| 偏移 | 长度 | 字段 | 说明 |
|---:|---:|---|---|
| 0 | 2 | `SOF` | 帧头，固�?`0xA5 0x5A` |
| 2 | 2 | `LEN` | �?`TYPE` �?`PAYLOAD` 结束的长度，低字节在�?|
| 4 | 1 | `TYPE` | 帧类�?|
| 5 | 2 | `SEQ` | 序号，低字节在前 |
| 7 | N | `PAYLOAD` | ASCII 载荷，格式为 `key=value,key=value` |
| 7+N | 2 | `CRC16` | CRC16-CCITT，低字节在前 |
| 9+N | 2 | `EOF` | 帧尾，固�?`0x0D 0x0A` |
...
1. 搜索 `0xA5 0x5A`�?
2. 读取 `LEN`�?
3. �?`LEN` 超过最大载荷限制，丢弃并重新找帧头�?
4. 读取 `TYPE + SEQ + PAYLOAD + CRC16 + EOF`�?
5. 校验 EOF 是否�?`0x0D 0x0A`�?
6. 校验 CRC�?
7. 解析 `PAYLOAD`�?
...
| TYPE �?| 名称 | 方向 | 说明 |
|---:|---|---|---|
| `0x01` | `CMD` | PC -> MCU | 上位机命�?|
| `0x02` | `ACK` | MCU -> PC | 命令成功确认 |
| `0x03` | `NACK` | MCU -> PC | 命令失败 |
| `0x04` | `STATE` | MCU -> PC | 当前状态快�?|
| `0x05` | `DATA` | MCU -> PC | 过程数据记录�?|
...
```text
LEN低字�? LEN高字�? TYPE, SEQ低字�? SEQ高字�? PAYLOAD...
```
...
注意：由�?`PAYLOAD` 是变长字段，不建议直接把整个帧映射为 C 结构体读写。推荐只定义头部�?
...
| `t` | uint32 | MCU tick，单�?ms |
| `cell` | uint8 | Cell 编号，`0` �?`1` |
| `ok` | uint8 | `1` 成功，`0` 失败 |
| `err` | int | 错误�?|
| `msg` | string | 简短错误文本，可�?|
...
- �?MCU 固定�?1s 上报，则该命令可以只作为 PC 内部状态，不一定下发�?
...
PAYLOAD="t=123456,cell=0,mode=NORMAL,owner=PC,running=1,target=37.5,command=36.2,current=35.8,t0=35.7,t1=35.9,duty=0.083,duty_outer=0.083,duty_inner=0.058,ratio=0.700,error=0,phase=0"
```
...
| `running` | `0` 停止，`1` 运行 |
| `target` | 用户目标温度 |
| `command` | 斜坡处理后送入 PID 的目标温�?|
| `current` | Cell 平均温度 |
| `t0` | Cell 内第 1 路温�?|
| `t1` | Cell 内第 2 路温�?|
| `duty` | outer/main duty, kept for compatibility |
| `duty_outer` | outer Peltier duty, same as PID output duty |
| `duty_inner` | inner Peltier duty, default `duty_outer * 0.7` |
| `ratio` | fixed inner/outer duty ratio |
| `error` | 面板/控制错误�?|
| `phase` | 程序升温阶段，普通模式为 `0` |

## 11. 数据记录�?

`DATA` 用于 Qt 记录过程曲线和导�?Excel/CSV�?

建议 MCU �?1s 发送一次：

```text
TYPE=DATA(0x05), SEQ=0,
PAYLOAD="t=123456,cell=0,mode=PROGRAM,phase=3,target=65.0,command=48.2,current=48.0,t0=47.9,t1=48.1,duty=0.126,duty_outer=0.126,duty_inner=0.088,ratio=0.700,error=0"
```
...
| `t0` | 通道0/2温度 |
| `t1` | 通道1/3温度 |
| `duty` | outer/main output duty, kept for compatibility |
| `duty_outer` | outer Peltier duty |
| `duty_inner` | inner Peltier duty |
| `ratio` | inner/outer duty ratio |
| `error` | 错误�?|
...
- PC 连续 3s 未收�?MCU 任何帧：显示通信断开�?
- MCU 连续 5s 未收�?PC 心跳：仅清除 `owner=PC` 的在线标志，不建议自动停止控温�?
- 如果产品安全策略要求 PC 掉线必须停止，可增加配置�?`pc_loss_action=STOP`�?
...
- PC 控制时，面板仍可停止�?
- 面板控制时，PC 也可停止�?
- 启动或修改参数时，如果另一个来源正在运行，MCU 可返�?`MODE_CONFLICT`，也可以允许接管�?

推荐初版使用“最后操作接管”：

```text
谁成功发送启�?设置命令，owner 就变成谁�?
```
...
- 不根据按钮点击直接认为状态已改变�?
- 点击后等�?`ACK` �?`STATE`�?
- 收到 `STATE` 后，�?`STATE` 覆盖 Qt 本地界面状态�?
- 收到面板触发�?`STATE/EVENT` 后，Qt 也必须同步更新�?

MCU 侧必须遵守：

- 任意来源修改状态后，都发�?`STATE`�?
- 任意来源启动/停止后，都发�?`EVENT` �?`STATE`�?
- 周期性发�?`STATE` �?`DATA`，防�?Qt 状态长期漂移�?
