...
static DRV8703_Status_t DRV8703_Transfer16(DRV8703_Handle_t *dev, uint16_t tx, uint16_t *rx)
{
    ...
    DRV8703_CsLow(dev);
    /*
     * DRV8703 CS setup time is tens of nanoseconds. A short busy loop
     * is sufficient and avoids the millisecond-level blocking of HAL_Delay
     * which would keep the SysStateMutex held longer than necessary.
     */
    {
        volatile uint32_t d;
        for (d = 0U; d < 10U; d++)
...
DRV8703_Status_t DRV8703_ClearFault(DRV8703_Handle_t *dev)
{
    ...
    /*
     * Write only CLR_FLT=1 without touching LOCK bits.
     * LOCK bits (bit5-3) only respond to 011b (unlock) or 110b (lock);
     * writing 000b (what the CLR_FLT-only value has in that field) is
     * ignored by the lock logic, so the register stays locked.
     */
    ret = DRV8703_WriteReg(dev,
                           DRV8703_REG_MAIN_CONTROL,
                           DRV8703_MAIN_CLR_FLT);
    if (ret != DRV8703_OK)
        return ret;
...
DRV8703_Status_t DRV8703_CheckFaultPins(DRV8703_Handle_t *dev)
{
    ...
    /*
     * Read the nFAULT pin multiple times to filter out switching noise.
     * Only report a fault if all samples agree the pin is active.
     */
    if (dev->cfg.fault_port != 0)
    {
        ...
        {
            GPIO_PinState level = HAL_GPIO_ReadPin(dev->cfg.fault_port, dev->cfg.fault_pin);
            if (dev->cfg.active_low_fault_pins != 0U)
                level = (level == GPIO_PIN_RESET) ? GPIO_PIN_SET : GPIO_PIN_RESET;
            if (level == GPIO_PIN_SET)
                fault_count++;
            /* Small delay between samples to let noise settle */
            {
                volatile uint32_t d;
                for (d = 0U; d < 50U; d++)
                    ;
    ...
    /*
     * Read the nWDFLT pin with the same filtering.
     */
    if (dev->cfg.wdflt_port != 0)
    {
        uint8_t wdflt_count = 0U;
        uint8_t samples = 5U;

        for (i = 0U; i < samples; i++)
        {
            GPIO_PinState level = HAL_GPIO_ReadPin(dev->cfg.wdflt_port, dev->cfg.wdflt_pin);
            if (dev->cfg.active_low_fault_pins != 0U)
                level = (level == GPIO_PIN_RESET) ? GPIO_PIN_SET : GPIO_PIN_RESET;
            if (level == GPIO_PIN_SET)
                wdflt_count++;
